...
Weibull  = lambda time,Lambda,Alpha,maxPower,minPower: minPower + (maxPower-minPower)*(1 - np.exp(-1*np.power((Lambda*(time-t0)),Alpha)))
...
#        r2Vec = r2_score(data[i],fit[i])
        
    if findPar:
        return np.average(FVU)
    else:
        return FVU
    ###############################################################################
...
for i in LambdaVec:
    for k in AlphaVec:
        ...
        fvuVEC = function(par,time,data,fit,False)
        ...
        plt.legend(['0 kRad','30 kRad - r2 = {:.3f}'.format((1-fvuVEC)[0]),'60 kRad - r2 = {:.3f}'.format((1-fvuVEC)[1]),'100 kRad - r2 = {:.3f}'.format((1-fvuVEC)[2])])
