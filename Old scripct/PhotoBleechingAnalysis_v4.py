...
#                [3,'6378Lf,10m,100kRad_set-1',19.68,16.179,0,'100 kRad set 1',True,True,True,0],
#                [3,'6378Lf,10m,100kRad_set-2',19.68,16.179,0,'100 kRad',False,True,False,0],              
#                [3,'6378Lf,10m,100kRad_set-3',19.68,16.179,0,'100 kRad',False,True,False,0],
...
futurMuliplier = 9.5 # Factor to multuply orginal dataset length
...
if setMaxValue:
    ...
    AccLossFunc = lambda time,a,b: minPower + (maxPower-minPower)*(1- np.exp(-1*np.power((a*(time-t0)),b)))
    ...
    Function = [#['Log function', Logfunc,2,[1,1],'dBm'],
                ...
                ['Weibull function',AccLossFunc,2,[0.09,0.23],[-300,-16]],  #(0.09,-0.23)   6.5 = 0.166,1
...
#            parBox(ax,np.array(pMat[j][i]),1.32,0.86-(0.15*j))
            parBox(ax,np.array(pMat[j][i]),.92,0.5-(0.15*j))
